...
const maxSlot = 10; // Maximum slot number
...
async function setImage(slot) {
    ...
    try {
        ...
        if (!response.ok) {
            throw new Error(`Image slot${slot} not found.`);
        }
        ...
        setImage(slot === maxSlot ? 1 : slot + 1);
    }
}

// Handle "Previous" button click
document.getElementById('prevImageBtn').addEventListener('click', () => {
    currentSlot = currentSlot > 1 ? currentSlot - 1 : maxSlot; // Loop back to maxSlot
    setImage(currentSlot);
});

// Handle "Next" button click
document.getElementById('nextImageBtn').addEventListener('click', () => {
    currentSlot = currentSlot < maxSlot ? currentSlot + 1 : 1; // Loop back to slot 1
    setImage(currentSlot);
});

// Add click events to image containers
document.getElementById('left-image-container').addEventListener('click', () => {
    currentSlot = currentSlot > 1 ? currentSlot - 1 : maxSlot; // Loop back to maxSlot
    setImage(currentSlot);
});

document.getElementById('right-image-container').addEventListener('click', () => {
    currentSlot = currentSlot < maxSlot ? currentSlot + 1 : 1; // Loop back to slot 1
    setImage(currentSlot);
});
