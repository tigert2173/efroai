...
async function setImage(slot) {
    ...
    // Check if the image exists before applying
    try {
        const response = await fetch(url, { method: 'HEAD' });

        if (response.status === 404) {
            // If the image is not found (404), skip this slot and continue to the next
            console.log(`Slot ${slot} not found (404). Skipping...`);
            currentSlot = (currentSlot < 10) ? currentSlot + 1 : 1; // Loop to the next slot
            setImage(currentSlot); // Recursively try the next slot
            return;
        }
